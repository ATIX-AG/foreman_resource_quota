...
module ForemanResourceQuota
  class ResourceQuota < ApplicationRecord
    ...
    has_many :resource_quotas_usergroups, class_name: 'ResourceQuotaUsergroup', inverse_of: :resource_quota,
      dependent: :destroy
    has_many :hosts, class_name: '::Host::Managed', through: :hosts_resources
    ...
    def utilization(exclude: [])
      ...
      self.hosts_resources.each do |host_resources_item|
    ...
    def hosts_resources_as_hash
      resources_hash = self.hosts.map(&:name).index_with { {} }
      self.hosts_resources.each do |host_resources_item|
        self.active_resources do |resource_name|
